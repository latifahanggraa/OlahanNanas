...
include "../../genral/config.php";
 ...
 if(isset($_POST['send'])){
   ...
   if(empty($title)){
    ...
    if(empty($stok)){
      $errorStok[]="Stok field is empty!";
    }

    // Penerapan Stored Procedure
    if(empty($errorTitle) && empty($errorDescriptions) && empty($errorPrice) && empty($errorImage1) && empty($errorImage2) && empty($errorImage3) && empty($errorStok)){
      $stmt = $connectSQL->prepare("CALL InsertProduct(?, ?, ?, ?, ?, ?, ?, ?)");
      $stmt->bind_param("sssssdii", $image_name, $image_name2, $image_name3, $title, $descriptions, $price, $categoryId, $stok);
      $stmt->execute();
...
<form method="POST" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
  ...
  <!-- input price -->
  <div class="form-group row">
      <label class="control-label col-md-3" for="inputstok">Stok</label>
      <div class="col-md-8">
      <input value="<?php echo $stok ?>" name='stok' type="number" class="form-control" id="inputstok" placeholder="stok">
      <?php if(!empty($errorStok)):  ?>
      <div class="alert alert-danger" role="alert"><?php echo $errorStok[0] ;?></div>
      <?php endif; ?>
    </div>
    </div>
